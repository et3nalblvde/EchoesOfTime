...
class CongratulationsScreen:
    def __init__(self, screen, exit_to_main_menu, current_level):
        ...
        for frame in range(self.background_image.n_frames):
            ...
            frame_data = pygame.image.frombuffer(self.background_image.convert("RGBA").tobytes(),
                                                 self.background_image.size, "RGBA")
        ...
        while self.running:
            for event in pygame.event.get():
                ...
                if action == "next_level":
                    if self.current_level == "level_2":  # Если завершен второй уровень
                        print("Starting level 3...")
                        pygame.init()
                        self.running = False
                        start_level_3(self.screen, self.exit_to_main_menu, self.exit_to_main_menu)
                    else:  # Если завершен первый уровень
