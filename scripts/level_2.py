...
level_2_image_path = os.path.join(base_folder, '..', 'assets', 'sprites', 'maps', 'level_2.jpg')
level_2_image = pygame.image.load(level_2_image_path)

def scale_background(screen):
    """Масштабирует фон под размер экрана."""
...
def update_level_status(level, status):
    """Обновляет статус уровня в файле настроек."""
...
def game_over_screen_loop(game_over_screen):
    """Запускает цикл экрана проигрыша."""
...
def start_level_2(screen, restart_main_menu, exit_to_main_menu):
    ...
    player = Player(200, 600, player_sounds)  # Начальные координаты для второго уровня
    shadow = Shadow(150, 800)  # Начальные координаты для тени
    health = Health(max_health=3, x=10, y=10, player=player)

    # Дверь
    door_x, door_y = 2400, 65
    ...
    clock = pygame.time.Clock()
    collision = CollisionLevel2()  # Используем коллизии для второго уровня
    running = True
    player_dead = False
    ...
    while running:
        ...
        # Логика взаимодействия с рычагами
        if controlling_player:
        ...
        if door.is_open and door.rect.colliderect(player.rect):
            ...
            result = congratulations_screen.congratulations_screen()
            if result == "next_level":
                from level_3 import start_level_3  # Предполагается, что есть третий уровень
